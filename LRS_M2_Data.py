...
hop_length          = n_fft // 2    # 75% overlap for stable temporal tracking
win_length          = n_fft         # Window length for STFT. 
                                    # We mostly use chroma_cqt, which doesn't use win_length.
...
WORK_META_TEMPLATE = {
    ...
    'original artist':      '',
    'language':             '',
    'oa type':              '',     # Original artist type.
    'os key':               '',     # Original starting key
    'rs key':               '',     # Recording starting key   
    'key modulation':       None,
    'source':               '',
    'file name':            '',
    'path original audio':  '',
...
# Need to set hop_length first so that frame_duration can be derived.
CHROMA_SETTINGS = {'hop_length': 1024}
...
CHROMA_SETTINGS = {
    ...
    'frame_hop_ms': CHROMA_SETTINGS['hop_length'] / EXPECT_SR * 1000,  # For reporting and thresholding
...
'trim music', 'Language', 'OA Type', 'OS Key', 'RS Key', 'Key Modulation']
"""
...
keys_should_be_number     = ['work year', 'release year', 'original version', 'official source', 
                             'trim music', 'key modulation']
...
def key_symbol_in_dict_valid(dict_meta, keys_with_key_symbol=['OS Key']):
    """
    Check if the values of keys_with_key_symbol in dict_meta are valid key symbols (e.g., C, D#, Fm, etc.).
    Return a dict with the validity of each key.
    Replace "♭" with "b" and "♯" with "#" before checking.
    """
    valid_keys = ['C', 'C#', 'Db', 'D', 'D#', 'Eb', 'E', 'F', 'F#', 'Gb', 'G', 'G#', 'Ab', 'A', 'A#', 'Bb', 'B',
                  'Cm', 'C#m', 'Dbm', 'Dm', 'D#m', 'Ebm', 'Em', 'Fm', 'F#m', 'Gbm', 'Gm', 'G#m', 'Abm', 
                  'Am', 'A#m', 'Bbm', 'Bm']
    list_values = []
    for key in keys_with_key_symbol:
        value = dict_meta[key]  # Show error if the key doesn't exist in dict_meta.
        _value = value.replace('♭', 'b').replace('♯', '#') if isinstance(value, str) else value
        
        #[3] If the value is different after replacement, update the dict_meta with the new value.
        if value != _value:
            dict_meta[key] = _value
            value = _value
        
        #[3] Check if the value is in valid_keys. If not, print a warning.
        if value not in valid_keys:
            print(f'\n⚠️ [Warning] The value "{value}" of key "{key}" is not a valid key symbol. Please check the metadata for this track.')
        
        #[5] Append the value to list_values for return.
        list_values.append(value)

            
    return dict_meta, list_values
...
class Full_track_ID_Check:
    ...
    def __init__(self):
        ...
        if len(self.list_track_ID_need_update) > 0:
            #[8] These work_IDs are priority to update for current experiments. 
            # Reorder if list_track_ID_need_update contains any of these work_IDs to make them appear first in the list for user to process. 
            work_ID_priority = '96OADL6F'
            self.list_track_ID_need_update = sorted(self.list_track_ID_need_update, key=lambda x: 0 if x.startswith(work_ID_priority) else 1)
            
            #[9] Ask user if they want to update now.
    ...
    def varify_recording_level(self):
        ...
        #[7] Print summary table of recording-level check.
        df_recd_check = df_recd_check.sort_values(by='track_ID').reset_index(drop=True)
        ...
        #[8] Collect recording_IDs that need update for later processing.
...
class NewSongRegistration:
    ...
    def from_audio_to_chroma(audio_path, path_chroma, dict_chroma_settings):
        """
        This is the wrapper function that takes an audio file path, 
        computes the chroma feature, and saves it as a .npz file.
        """
...
def compute_chroma_feature(audio_mono, sr, dict_chroma_settings):
    ...
    # [2] Compute chroma feature (2026-0331-1036: chroma_cqt for now.)
